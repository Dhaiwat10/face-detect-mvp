...
electron_1.app.disableHardwareAcceleration(); // To fix rasterization errors on some systems
electron_1.app.whenReady().then(async () => {
    const userDataPath = electron_1.app.getPath('userData');
    const db = (0, database_1.initializeDatabase)(userDataPath);
    (0, database_1.setupDatabase)(db); // Note: This clears the DB on every start. Consider changing this behavior.
...
electron_1.ipcMain.handle('index-folder', async () => {
    if (!mainWindow)
        return;
    ...
    const knownFaces = db.prepare('SELECT id, descriptor FROM persons').all();
    ...
    let faceMatcher = labeledFaceDescriptors.length > 0 ? new faceapi.FaceMatcher(labeledFaceDescriptors) : null;
    ...
    for (const imageFile of imageFiles) {
        ...
        const results = await faceapi.detectAllFaces(img, detectionOptions).withFaceLandmarks().withFaceDescriptors();
        if (!results.length)
            continue;
...
electron_1.ipcMain.handle('query-by-image', async () => {
    if (!mainWindow)
        return;
    const { canceled, filePaths } = await electron_1.dialog.showOpenDialog(mainWindow, {
        properties: ['openFile'],
